...
function createProfileButton() {      
    ...
    button.innerHTML = `      
        <img src="${window.Auth && window.Auth.userData ? (window.Auth.userData().foto_profil || generateGitHubAvatar(window.Auth.currentUser().email)) : ''}"       
             alt="Profile"       
             class="profile-image"      
...
function createProfilePanel() {      
    ...
    panel.innerHTML = `      
        <div class="profile-header">      
            <h2>${window.Auth && window.Auth.profileState && window.Auth.profileState().isProfileComplete ? 'Profil Saya' : 'Lengkapi Profil'}</h2>      
        ...
        <div class="profile-content">      
            <div class="current-profile">      
                <img src="${window.Auth && window.Auth.userData ? (window.Auth.userData().foto_profil || generateGitHubAvatar(window.Auth.currentUser().email)) : ''}"       
                     ...
                     onerror="this.src='https://api.dicebear.com/7.x/avataaars/svg?seed=user&backgroundColor=6b7280'">      
                <div class="current-name">${window.Auth && window.Auth.userData ? (window.Auth.userData().nama || window.Auth.currentUser().displayName || 'Nama belum diisi') : 'Nama belum diisi'}</div>      
            </div>      
            ...
            <div class="edit-section">      
                <div class="name-input-group">      
                    ...
                    <input type="text"       
                           ...
                           value="${window.Auth && window.Auth.userData ? (window.Auth.userData().nama || '') : ''}">      
                </div>      
...
function populateAvatarOptions() {      
    ...
    const avatars = window.Auth ? window.Auth.DEFAULT_AVATARS : [];    
...
function checkForChanges() {      
    const nameChanged = window.Auth ? (window.Auth.profileState().tempName !== (window.Auth.userData().nama || '')) : false;      
    ...
    if (window.Auth) {    
        ...
        state.hasChanges = nameChanged || avatarChanged;    
        window.Auth.profileState = () => state;    
    }    
          
    const saveBtn = document.getElementById('saveProfile');      
    if (saveBtn) {      
        const isLoading = window.Auth ? window.Auth.profileState().isLoading : false;    
        const hasChanges = window.Auth ? window.Auth.profileState().hasChanges : false;    
...
async function saveProfile() {      
    ...
    try {      
        ...
        state.isProfileComplete = willBeComplete;    
...
// Helper function untuk generateGitHubAvatar (duplikat karena digunakan di UI)    
