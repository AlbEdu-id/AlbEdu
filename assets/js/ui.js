...
UI.CSSLoader = {
    ...
    loadProfileCSS: function() {
        if (this.loaded || this.loading) return Promise.resolve();
        
        return new Promise((resolve, reject) => {
            this.loading = true;
            
            // Coba beberapa path untuk kompatibilitas
            const cssPaths = [
            ...
            const tryLoadCSS = (path, index) => {
                ...
                link.onload = () => {
                    console.log(`✅ CSS Profile dimuat dari: ${path}`);
                    this.loaded = true;
                    this.loading = false;
                    resolve();
                };
                ...
                link.onerror = () => {
                    loadedCount++;
                    lastError = `Gagal memuat CSS dari: ${path}`;
                    
                    if (loadedCount === cssPaths.length) {
                        console.warn('⚠️ Semua path CSS gagal, menggunakan fallback inline CSS');
                        this.loading = false;
                        this.injectFallbackCSS();
                        resolve();
                    } else if (index < cssPaths.length - 1) {
                        // Coba path berikutnya
                        setTimeout(() => tryLoadCSS(cssPaths[index + 1], index + 1), 100);
                    }
                };
            ...
            tryLoadCSS(cssPaths[0], 0);
        });
    },
    
    injectFallbackCSS: function() {
        if (document.querySelector('#profile-css-fallback')) return;
        
        const style = document.createElement('style');
        style.id = 'profile-css-fallback';
        style.textContent = `
            /* Fallback CSS - Minimal styling untuk fungsi dasar */
            .profile-overlay {
                position: fixed;
                top: 0;
                left: 0;
            ...
            .view-mode, .edit-mode, .edit-avatar-mode {
                padding: 24px;
                overflow-y: auto;
                flex: 1;
            }
        ...
        console.log('✅ Fallback CSS disuntikkan');
        this.loaded = true;
        this.loading = false;
    }
};
...
UI.Profile = {
    init: async function() {
        ...
        try {
            ...
            await UI.CSSLoader.loadProfileCSS();
            console.log('✅ CSS Profile berhasil dimuat');
            
            // Tunggu sebentar untuk memastikan CSS diterapkan
            ...
            console.error('❌ Gagal menginisialisasi Profile System:', error);
        }
    },
